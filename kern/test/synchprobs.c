...
static bool failif(bool condition, const char* message)
{
  if (condition) {
 ...
 */
static void initialize_thread(volatile void* threads[], uint32_t index)
{
 ...
 */
static void check_thread(volatile void* threads[], uint32_t index)
{
  failif((threads[index] != curthread->t_stack),
         "failed: incorrect thread type");
}
 ...
 */
static void check_role(uint32_t index, int role)
{
  failif((whale_roles[index] != role), "failed: incorrect role");
}

static void male_wrapper(void* unused1, unsigned long index)
{
  (void)unused1;
...
static void female_wrapper(void* unused1, unsigned long index)
{
  (void)unused1;
...
static void matchmaker_wrapper(void* unused1, unsigned long index)
{
  (void)unused1;
...
int whalemating(int nargs, char** args)
{
  ...
  for (i = 0; i < CHECK_TIMES && loop_status == TEST161_SUCCESS; i++) {
    ...
    if ((male_start_count != NMATING) || (female_start_count != NMATING) ||
        (matchmaker_start_count + male_end_count + female_end_count +
             matchmaker_end_count !=
         0)) {
  ...
  if (failif((loop_status == TEST161_FAIL),
  ...
  failif((max_concurrent_matchmakers == 1),
         "failed: no matchmaker concurrency");
...
static void initialize_car_thread(uint32_t index, uint32_t direction,
                                  uint32_t turn)
{
 ...
 * It first checks current intersection status make sure no more than one car in
 * one quadrant. Then it removes current car from previous location. In the end,
 * it returns current car's index for inQuadrant, to let inQuadrant update
 * car_locations array.
 */
static int move(uint32_t index)
{
...
static void turnright_wrapper(void* index, unsigned long direction)
{
  random_yielder(4);
  lock_acquire(testlock);
...
static void gostraight_wrapper(void* index, unsigned long direction)
{
  random_yielder(4);
  lock_acquire(testlock);
...
static void turnleft_wrapper(void* index, unsigned long direction)
{
  random_yielder(4);
  lock_acquire(testlock);
...
int stoplight(int nargs, char** args)
{
  ...
  if ((test_status == TEST161_SUCCESS) &&
      ...
      (!(failif((all_quadrant != required_quadrant),
                "failed: didn't do the right turns"))) &&
